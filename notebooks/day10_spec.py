...
print('검색기를 한국어 모델로 바꿨다')"""),
    ...
    md("### 왜 번호와 전문용어에 약한가"),
    md("모델은 글자를 통째로 보지 않는다. **먼저 조각으로 쪼갠다.**\n"
       "자주 나온 말은 한 덩어리로 남고, 드문 말은 여러 조각으로 흩어진다."),

    prep("""# 검색에 쓰는 모델이 낱말을 어떻게 쪼개는지 직접 본다
...
for w in ['휴가', '연차', '제60조', '제61조', 'ERR-5010']:
    ...
    md("`휴가` 는 **한 조각**이라 뜻이 온전히 남는다.\n"
       "`제60조` 는 `제 / ##60 / ##조` **세 조각**이 되고, `제61조` 는 `제 / ##61 / ##조` 다.\n"
       "**두 낱말이 세 조각 중 두 개를 공유한다.** 그래서 좌표가 거의 붙어 버린다.\n"
       "앞에서 「제60조」로 검색했을 때 엉뚱한 조문이 온 이유가 이것이다."),

    md("### 현장 용어는 더 심하다"),

    code("""# 이차전지 현장에서 쓰는 말로 해 본다
for w in ['애노드', '캘린더링', 'NCM811', 'SEI층', '화성공정', 'NMP', '전극', '리튬']:
    t = tk.tokenize(w)
    print('%-10s %d조각  %s' % (w, len(t), t))"""),

    md("`리튬` 은 한 조각인데 `NCM811` 은 `NC / ##M / ##81 / ##1` **네 조각**이다.\n"
       "`캘린더링` 은 `캘 / ##린 / ##더 / ##링` 으로 글자 단위까지 부서진다.\n\n"
       "**모델은 이 말을 배운 적이 없다.** 학습 자료에 드물었기 때문이다.\n"
       "그러면 「NCM811 코팅 조건」으로 검색해도 그 말이 든 문서를 못 집는다.\n\n"
       "| 이럴 때 | 이렇게 |\n|---|---|\n"
       "| 제품 코드 · 설비 번호 · 규격 | **낱말 검색**을 같이 돌린다 |\n"
       "| 현장 용어가 많은 문서 | 용어를 **메타데이터**로 따로 붙인다 |\n"
       "| 사내 용어가 아주 많다면 | 그 문서로 임베딩 모델을 **추가 학습**한다 |"),

    md("### 쪼개지는 말은 어떻게 다루나"),
    md("가장 손쉬운 것부터 해 본다. **질문에 뜻을 덧붙이는 것**이다.\n"
       "코드나 약어 옆에 풀어 쓴 말을 붙이면, 그 말이 좌표를 끌어 준다."),

    prep("""# 현장 용어 사전 — 코드 옆에 무슨 뜻인지 적어 둔다
GLOSSARY = {
    '제60조': '연차 유급휴가',
...
print(expand('제60조'))"""),
    ...
    code("""# 덧붙이기 전과 후를 견준다
for q in ['제60조', expand('제60조'), '근로기준법 제60조 연차 유급휴가 며칠']:
    e = rank_of(V @ EMB.encode([q], normalize_embeddings=True)[0], '근로기준법', '제60조')
    t = rank_of(cosine_similarity(vec.transform([q]), M)[0], '근로기준법', '제60조')
    print('%-34s 의미 %4s등   낱말 %4s등' % (q, e, t))"""),

    md("**201등이 13등이 되고, 문장으로 풀어 쓰면 1등**이 된다.\n"
       "코드만 던지면 못 찾지만, 그 코드가 무슨 뜻인지 한 마디 붙이면 찾는다.\n"
       "사전은 엑셀 한 장이면 된다. **현장 용어를 아는 사람이 채우는 자리**다."),

    md("### 네 가지 방법과 드는 품"),
    md("| 방법 | 무엇을 하나 | 품 | 언제 |\n|---|---|---|---|\n"
       "| **질문에 뜻 붙이기** | 용어 사전으로 검색어를 넓힌다 | 사전 한 장 | 여기서 시작한다 |\n"
       "| **낱말 검색 같이 돌리기** | 코드·번호는 글자로 잡는다 | 코드 몇 줄 | 제품 코드 · 설비 번호가 많을 때 |\n"
       "| **용어를 메타데이터로** | 조각에 `terms: [NCM811]` 을 달고 필터한다 | 문서 손질 | 용어가 정해져 있을 때 |\n"
       "| 임베딩 모델 추가 학습 | 사내 문서로 모델을 더 배우게 한다 | GPU · 데이터 | 앞의 셋으로 안 될 때 |\n\n"
       "위에서 아래로 갈수록 품이 든다. **위의 둘로 대부분 해결된다.**"),

    md("> 토크나이저에 용어를 <b>추가만</b> 하는 것(<code>add_tokens</code>)은 도움이 안 된다.\n"
       "> 한 조각으로 묶이기는 하지만 그 조각의 좌표가 **아무 뜻도 없는 값**이라, 추가 학습을 같이 해야 한다."),
