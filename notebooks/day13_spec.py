...
for k, v in 여섯칸.items():
    print('%-10s %s' % (k, v))""",
         check="""assert '___' not in str(여섯칸), '여섯 칸을 다 채운다'
print()
print('빈 칸 없음. 다음 문제에서 이것을 프롬프트로 옮긴다.')"""),

    md("## 3. 프롬프트로 옮기기"),
    md("여섯 칸을 **문장으로** 바꾼다. 아래 뼈대에 값만 끼워 넣으면 된다."),

    Ex(1, "여섯 칸을 프롬프트로 만든다. **빠뜨리면 안 되는 두 줄**이 뒤에 있다.\n"
