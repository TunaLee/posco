...
CELLS = [
    md("## 1. 노트북 밖으로"),
    md("어제 만든 서버는 **노트북 안에** 있었다. `Client(mcp)` 로 붙었다.\n"
       "노트북을 닫으면 사라지고, 나 말고는 아무도 못 쓴다.\n\n"
       "오늘은 그것을 **파일로 떼어 내고, 진짜로 띄우고, 남이 붙게** 한다."),
...
print('%%d행 %%d열' %% df.shape)""" % BASE),
    ...
    md("### 서버를 파일로 쓴다"),
    md("아래 글자를 그대로 `server.py` 에 적는다. 어제 노트북에 있던 것과 몸통이 같다."),
...
print(open('server.py', encoding='utf-8').read()[:300])""" % BASE),

    md("**맨 아래 `mcp.run()` 이 어제와 갈리는 자리다.** 이 줄이 서버를 돌린다.\n"
       "`__name__ == \"__main__\"` 은 「이 파일을 직접 실행했을 때만」이라는 뜻이다."),

    md("## 2. 띄우면 주소가 생긴다"),
    md("`mcp.run()` 은 **셀을 붙잡고 안 놓는다.** 서버는 원래 계속 도는 것이라 그렇다.\n"
       "노트북에서 이어서 쓰려면 **딴 갈래로 띄운다.**"),

    prep("""# 같은 서버를 이번엔 HTTP 로 띄운다. 딴 갈래로 돌려야 셀이 안 막힌다.
srv = FastMCP('공정 도우미')
...
print('서버가 떴다 —', ADDR)"""),
    ...
    md("**주소가 생겼다.** 이제 이 서버는 노트북 안의 변수가 아니라 **주소가 있는 것**이다."),

    md("## 3. 주소로 붙는다"),
    md("어제는 서버를 담은 변수를 넘겼다. 오늘은 **주소만** 넘긴다."),

    prep("""# 괄호 안만 바뀐다. 그 뒤는 어제와 한 글자도 안 다르다.
...
async with Client('http://127.0.0.1:%d/mcp' % PORT2) as c:
    ...
    md("## 4. 모델이 보는 것"),
    md("도구를 만들면 모델에게 무엇이 넘어가는지부터 본다. **여기 없는 것은 모델이 모른다.**"),
...
def chat(messages, tools=None, n=600):
    ...
    code("""# 서버가 모델에게 넘기는 글을 그대로 찍어 본다
...
print('스키마 ', json.dumps(t.inputSchema, ensure_ascii=False, indent=2))"""),
    ...
    md("**이름 · 설명 · 스키마 셋이 전부다.** 함수 몸통은 안 넘어간다.\n\n"
       "그래서 코드가 아무리 정확해도 **이름과 설명이 흐리면** 모델은 못 고른다.\n"
       "설명은 사람이 아니라 **모델이 읽는 글**이다."),

    prep("""# 어제 만든 고리 그대로. 클라이언트가 목록을 받고, 모델이 고른다.
def to_openai(tools):
...
async def run(target, question, log=True):
    ...
    md("## 5. 고르는 것은 흔들린다"),
    md("도구가 둘 이상이면 모델이 **고른다.** 그런데 이 고르기는 매번 같지 않다.\n"
       "같은 질문을 세 번 던져 본다."),

    prep("""# 이름이 비슷한 도구 둘을 한 서버에 둔다
pick = FastMCP('고르기 시험')
...
print('도구 둘을 붙였다')"""),
    ...
    code("""# 뜻이 분명한 질문과 모호한 질문을 각각 세 번씩
for q in ['3호기 불량률 알려줘', '3호기 상태 좀 봐줘']:
    ...
    md("**「불량률」처럼 뜻이 분명하면 세 번 다 같은 도구가 불린다.**\n"
       "**「상태 좀 봐줘」처럼 모호하면 매번 다르다.** 온도를 0 으로 놓아도 그렇다.\n\n"
       "그래서 도구는 **좁게, 이름을 분명하게** 만든다.\n"
       "한 도구가 여러 일을 하면 모델이 언제 부를지 판단할 근거가 없다."),

    Ex(2, "도구 이름을 **분명하게** 바꿔 본다. `lookup` 은 무엇을 하는지 안 보인다.\n"
          "> 이름만으로 무엇을 돌려주는지 알 수 있게 짓는다.",
       setup="""vague = FastMCP('이름이 흐린 서버')
""",
       blank="""@vague.tool()
def ___(machine: str) -> str:
...
def compare_shifts(machine: str) -> str:
    ...
    return ' · '.join('%s %.1f%%' % (s, 100.0 * (g['판정'] == '불량').mean())
                      for s, g in d.groupby('교대조'))""",
       check="""answer, called = await run(vague, '3호기는 주간과 야간 중 어느 쪽이 불량이 많나', log=False)
print('부른 도구:', called or '없음')
print(answer[:150])"""),

    md("## 6. 나쁜 입력이 왔을 때"),
    md("모델은 **없는 설비 이름**을 넣기도 한다. 그때 도구가 무엇을 돌려주느냐로 갈린다.\n"
       "먼저 도구를 직접 불러 **모델이 받게 될 글**을 본다."),

    prep("""# 검증이 없는 도구와, 길을 알려 주는 도구
...
print('길을 알려 줌 :', await raw(kind, '7호기'))"""),
    ...
    md("> 검증이 없는 쪽은 **빨간 오류 상자**가 길게 찍힌다. 서버가 남기는 기록이라 그렇다.\n"
       "> 놀랄 것 없다. 우리가 볼 것은 그 아래 한 줄이다."),

    md("**이 두 줄이 모델에게 넘어가는 글이다.**\n\n"
       "앞엣것은 무엇이 잘못됐는지 안 알려 준다. 뒤엣것은 **쓸 수 있는 이름을 같이 준다.**\n"
       "에러 문구는 사람만 읽는 것이 아니다."),

    code("""# 모델에게 맡기면 어떻게 되나
for name, server in [('검증 없음', rude), ('길을 알려 줌', kind)]:
    ...
    md("> **답에 「가상 데이터」나 「예시」 같은 말이 섞이면 위험 신호다.**\n"
       "> 도구가 값을 안 주면 모델은 **지어내서라도** 답하려 든다.\n"
       "> 없으면 없다고 돌려주는 것이 도구의 몫이다."),

    md("## 6-1. 어디까지 열 것인가"),
    md("여기까지는 **읽기만** 하는 도구였다. 쓰는 도구를 붙이는 순간 성격이 달라진다.\n"
       "읽기는 틀려도 되돌릴 수 있지만, 쓰기는 안 된다."),

    prep("""# 읽기 도구와 쓰기 도구를 표시로 갈라 둔다
memo = FastMCP('점검 메모')
NOTES = []
...
def recent_lots(machine: str, n: int = 5) -> str:
    ...
    n: 몇 건. 최대 20
    \'\'\'""",
       blank="""    d = df[df['설비호기'] == machine].tail(min(n, ___))
    return d[___].to_string(index=False) if len(d) else '해당 설비가 없다'""",
       answer="""    d = df[df['설비호기'] == machine].tail(min(n, 20))
    return d[OPEN].to_string(index=False) if len(d) else '해당 설비가 없다'""",
       check="""SHUT = [c for c in df.columns if c not in OPEN]
...
print('돌려준 줄 수:', out.count(chr(10)))"""),
    ...
    md("## 7. 서버가 여럿일 때"),
    md("조마다 서버를 만들면 **한 앱에 여럿을 붙여야** 한다. 여기서 사고가 난다."),

    prep("""# 두 조가 각각 서버를 냈다. 그런데 도구 이름이 겹친다.
team1 = FastMCP('1조 설비')
@team1.tool()
def defect_rate(machine: str) -> str:
    '''설비호기의 불량률을 돌려준다'''
...
async with Client(plain) as c:
    ...
    md("**1조 도구가 사라졌다.** 경고 한 줄이 로그에 찍힐 뿐 목록에서는 그냥 없다.\n"
       "부르면 2조 것이 나온다. 1조는 자기 도구가 안 불린다는 것도 모른다."),

    code("""# 접두어를 붙이면 둘 다 산다
...
async with Client(merged) as c:
    ...
    md("**도구 이름은 앱 안에서 전역이다.** 서버가 달라도 이름이 같으면 부딪힌다.\n"
       "붙이는 쪽이 접두어를 붙여 갈라 준다."),

    md("## 8. 컨테이너로 싸기"),
    md("서버가 **내 노트북에서만** 도는 문제가 남았다.\n"
       "파이썬 버전, 깔린 꾸러미, 데이터 경로가 사람마다 다르다.\n\n"
       "컨테이너는 **그 전부를 한 덩이로 싸는** 것이다."),

    prep("""# 서버가 쓰는 꾸러미를 적어 둔다
...
print(open('Dockerfile').read())"""),
    ...
    md("### 여럿을 한 번에 — compose"),
    md("조가 셋이면 서버가 셋이다. 하나씩 띄우고 포트를 외우는 것은 오래 못 간다.\n"
       "**compose 는 여럿을 한 파일에 적고 한 번에 올린다.**"),

    prep("""# 호스트 하나에 조별 서버 둘을 붙이는 구성
COMPOSE = '''
services:
  team1:
    build: ./teams/team1
...
print(open('compose.yml').read())"""),
    ...
    md("**`http://team1:8000/mcp`** &mdash; 여기서 `team1` 은 기계 이름이 아니라 **서비스 이름**이다.\n"
    ...
    md("### 사내망에서 쓰려면"),
    md("바깥 인터넷이 막힌 곳에서는 위 그대로는 안 된다. `FROM python:3.12-slim` 부터 못 받는다.\n"
       "막히는 자리가 셋이고, 자리마다 푸는 법이 있다."),

    prep("""# ① 이미지를 사내 저장소에서 받는다  ② 꾸러미도 사내 미러에서 받는다
INTRA = '''
...
print(open('Dockerfile.intra').read())"""),
    ...
    md("**③ 저장소도 미러도 없으면** 바깥에서 만들어 파일로 옮긴다.\n\n"
       "```\n"
    ...
    md("> 프록시만 있는 곳이라면 빌드할 때 넘겨준다 &mdash;\n"
       "> `docker build --build-arg HTTP_PROXY=$HTTP_PROXY --build-arg HTTPS_PROXY=$HTTPS_PROXY .`"),

    Ex(4, "compose 에 **3조 서버**를 한 칸 더 넣는다.\n"
          "> 호스트가 3조를 찾아가게 `TEAM_URLS` 도 같이 고친다.",
       setup="import re\nC = open('compose.yml').read()",
       blank="""C = C.replace('''  host:''', '''  team3:
    build: ./teams/team3
    expose: ["___"]

  host:''')
C = C.replace('http://team2:8000/mcp"', 'http://team2:8000/mcp,___"')
C = C.replace('depends_on: [team1, team2]', 'depends_on: [team1, team2, ___]')""",
       answer="""C = C.replace('''  host:''', '''  team3:
    build: ./teams/team3
    expose: ["8000"]
...
print('3조까지 붙는 구성이 됐다')"""),
    ...
    md("## 9. 우리 팀 서버"),
    md("발표에 낼 것을 여기서 정한다. **호스트는 준비돼 있다.**\n"
       "조는 `server.py` 하나만 내면 채팅창에 붙는다."),

    Task(1, "**우리 조 서버**를 설계한다. 코드는 안 쓴다. 다섯 칸만 채운다.\n"
...
SPEC = ("MCP 서버 만들기 · 내 앱에 붙이기",
        "노트북 안에 있던 서버를 파일로 떼어 내고, 띄우고, 남이 붙게 한다", CELLS, MODES)
